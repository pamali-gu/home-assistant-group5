...
class AlexaModeController(AlexaCapability):
    ...
    def capability_resources(self) -> dict[str, list[dict[str, Any]]]:
        ...
        # Instance mappings for simpler cases with lambda and callable type hint
        instance_mapping: dict[str, Callable[[], dict[str, list[dict[str, Any]]]]] = {
            f"{fan.DOMAIN}.{fan.ATTR_DIRECTION}": lambda: self._get_mode_resource(
                [AlexaGlobalCatalog.SETTING_DIRECTION],
                [fan.DIRECTION_FORWARD, fan.DIRECTION_REVERSE],
                fan.ATTR_DIRECTION,
            ),
            f"{fan.DOMAIN}.{fan.ATTR_PRESET_MODE}": lambda: self._get_mode_resource(
                [AlexaGlobalCatalog.SETTING_PRESET],
                self.entity.attributes.get(fan.ATTR_PRESET_MODES) or [],
                fan.ATTR_PRESET_MODE,
                na_mode_required=True,
            ),
            f"{humidifier.DOMAIN}.{humidifier.ATTR_MODE}": lambda: self._get_mode_resource(
                [AlexaGlobalCatalog.SETTING_MODE],
                self.entity.attributes.get(humidifier.ATTR_AVAILABLE_MODES) or [],
                humidifier.ATTR_MODE,
                na_mode_required=True,
            ),
            f"{water_heater.DOMAIN}.{water_heater.ATTR_OPERATION_MODE}": lambda: self._get_mode_resource(
                [AlexaGlobalCatalog.SETTING_MODE],
                self.entity.attributes.get(water_heater.ATTR_OPERATION_LIST) or [],
                water_heater.ATTR_OPERATION_MODE,
                na_mode_required=True,
            ),
            f"{remote.DOMAIN}.{remote.ATTR_ACTIVITY}": lambda: self._get_mode_resource(
                [AlexaGlobalCatalog.SETTING_MODE],
                self.entity.attributes.get(remote.ATTR_ACTIVITY_LIST) or [],
                remote.ATTR_ACTIVITY,
                na_mode_required=True,
            ),
            f"{cover.DOMAIN}.{cover.ATTR_POSITION}": lambda: self._get_mode_resource(
                ["Position", AlexaGlobalCatalog.SETTING_OPENING],
                [cover.STATE_OPEN, cover.STATE_CLOSED, "custom"],
                cover.ATTR_POSITION,
            ),
        }

        # Check for instance mapping and return the resource
        if self.instance in instance_mapping:
            return instance_mapping[self.instance]()

        # Handle valve position separately due to unique structure
        if self.instance == f"{valve.DOMAIN}.state":
            ...
            return self._get_mode_resource(
                ["Preset", AlexaGlobalCatalog.SETTING_PRESET],
                modes,
                "state",
                na_mode_required=len(modes) == 1,
            )
    ...
    def _get_mode_resource(
        self,
        labels: list[str],
        modes: list[str],
        attribute_name: str,
        na_mode_required: bool = False,
    ) -> dict[str, list[dict[str, Any]]]:
        """Create mode resources and add modes."""
        self._resource = AlexaModeResource(labels, False)
        for mode in modes:
            self._resource.add_mode(f"{attribute_name}.{mode}", [mode])
        if na_mode_required and len(modes) == 1:
            self._resource.add_mode(
                f"{attribute_name}.{PRESET_MODE_NA}", [PRESET_MODE_NA]
            )
...
class AlexaRangeController(AlexaCapability):
    ...
    def get_property(self, name: str) -> Any:
        ...
        attribute = self._get_instance_attribute()
        return self._fetch_attribute_value(attribute)
    ...
    def capability_resources(self) -> dict[str, list[dict[str, Any]]]:
        ...
        # Define the instance mapping with lambda and callable type hint
        instance_mapping: dict[str, Callable[[], dict[str, list[dict[str, Any]]]]] = {
            f"{fan.DOMAIN}.{fan.ATTR_PERCENTAGE}": lambda: self._create_alexa_preset(
            ...
            f"{humidifier.DOMAIN}.{humidifier.ATTR_HUMIDITY}": lambda: self._create_alexa_preset(
            ...
            f"{cover.DOMAIN}.{cover.ATTR_POSITION}": lambda: self._create_alexa_preset(
                ["Position", AlexaGlobalCatalog.SETTING_OPENING],
                min_value=0,
                max_value=100,
                precision=1,
            ...
            f"{cover.DOMAIN}.tilt": lambda: self._create_alexa_preset(
            ...
            f"{valve.DOMAIN}.{valve.ATTR_POSITION}": lambda: self._create_alexa_preset(
    ...
    def _get_fan_percentage(self) -> Any:
        """Return fan percentage or default to 100/0 based on state."""
        supported = self.entity.attributes.get(ATTR_SUPPORTED_FEATURES, 0)
    ...
    def _get_vacuum_fan_speed(self) -> int | None:
        """Return vacuum fan speed index based on speed list."""
    ...
    def _get_humidifier_target_humidity(self) -> Any:
        """Return humidifier target humidity.
    ...
    def _fetch_state_value(self) -> Any:
        """Return entity state for input_number and number domains."""
        if self.instance in (
    ...
    def _get_instance_attribute(self) -> str | None:
        """Map and return instance to corresponding attribute."""
        instance_mapping_dict = {
            f"{cover.DOMAIN}.{cover.ATTR_POSITION}": cover.ATTR_CURRENT_POSITION,
            f"{cover.DOMAIN}.tilt": cover.ATTR_CURRENT_TILT_POSITION,
            f"{fan.DOMAIN}.{fan.ATTR_PERCENTAGE}": fan.ATTR_PERCENTAGE,
            f"{humidifier.DOMAIN}.{humidifier.ATTR_HUMIDITY}": humidifier.ATTR_HUMIDITY,
            f"{input_number.DOMAIN}.{input_number.ATTR_VALUE}": None,
            f"{number.DOMAIN}.{number.ATTR_VALUE}": None,
            f"{vacuum.DOMAIN}.{vacuum.ATTR_FAN_SPEED}": vacuum.ATTR_FAN_SPEED,
    ...
    def _fetch_attribute_value(self, attribute: str | None) -> Any:
        """Return the appropriate attribute value based on instance."""
        if not attribute:
            return self._fetch_state_value()
        ...
        if self.instance == f"{humidifier.DOMAIN}.{humidifier.ATTR_HUMIDITY}":
            return self._get_humidifier_target_humidity()
        return self.entity.attributes.get(attribute)

    def _create_alexa_preset(
        self,
        labels: list[str],
        min_value: float,
        max_value: float,
        ...
        """Create and serialize AlexaPresetResource."""
    ...
    def _get_input_number_value_resource(self) -> dict[str, list[dict[str, Any]]]:
        """Return resource for input number value."""
    ...
    def _get_number_value_resource(self) -> dict[str, list[dict[str, Any]]]:
        """Return resource for number value."""
    ...
    def _get_vacuum_fan_speed_resource(self) -> dict[str, list[dict[str, Any]]]:
        """Return resource for vacuum fan."""
